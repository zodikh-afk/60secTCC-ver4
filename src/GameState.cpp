...
void GameState::SetConsumptionStrategy(
    std::unique_ptr<IConsumptionStrategy> strategy)
{
    consumptionStrategy = std::move(strategy);
}

void GameState::ConsumeDailyResources()
{
    if (!consumptionStrategy)
        return;

    // IPlayer → Player (у тебе реально Player)
    if (auto* p = dynamic_cast<Player*>(&player)) {
        consumptionStrategy->Consume(*p);
    }
}
...
{
    consumptionStrategy = std::make_unique<NormalConsumptionStrategy>();
    ...
    if (consumptionStrategy) {
    consumptionStrategy->Consume(*dynamic_cast<Player*>(&player));
    }
